#Commit all files in the category
...
if [ -d ".git" ]; then
  git add --all .
  git commit -m"Commit changes on repository $msgWithCommit" -- ./MechatronicBeing
  git push --progress "origin" main:main

  echo "[$0] ADD, COMMIT, PUSH on $categoryName repository."
fi
